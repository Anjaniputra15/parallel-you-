...
function useTypingAnimation(text: string, isActive: boolean, speed = 22) {
...
export default function PremiumDebatePage() {
  ...
  const isSpeakingA = loading && (!latestMsg || latestMsg.role === "persona_b");
  const isSpeakingB = loading && latestMsg?.role === "persona_a";

  const { displayed: typedText, isDone: typingDone } = useTypingAnimation(
    latestMsg?.content || "",
    phase === "debating" && !!latestMsg,
    18
  );
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Sora:wght@300;400;600;700&family=Source+Serif+4:ital,wght@0,400;0,600;1,400&family=DM+Mono:wght@400;500&display=swap');
        ...
        <div style={{
          ...
          {(phase === "debating" || phase === "paused") && (
            <>
              ...
              />
              <ControlButton icon="✋" label="Push Back" onClick={() => callTurn("pushback", "...")} color={T.red} />
              <ControlButton icon="🔍" label="Clarify" onClick={() => callTurn("clarify")} color={T.personaB} />
